			...
			this->m_data["m_players"] = nlohmann::json::array();
				...
				for(int i=0;i<64;i++)
				{
					if(players[i].Radar_Draw)
					{
						...
						this->m_data["m_local_team"] = players[LocalPlayerControllerIndex].TeamID_Radar;
						...
						this->m_player_data["m_steam_id"] = std::to_string(players[i].steamID_Radar);
						...
						this->m_player_data["m_has_bomb"] = false;
